...
//
#ifndef G4SaG4nParticleHPProduct_h
#define G4SaG4nParticleHPProduct_h 1
...
#include "G4SaG4nParticleHPDiscreteTwoBody.hh"
#include "G4SaG4nParticleHPIsotropic.hh"
#include "G4SaG4nParticleHPNBodyPhaseSpace.hh"
#include "G4SaG4nParticleHPLabAngularEnergy.hh"
#include "G4Cache.hh"
class G4ParticleDefinition;

enum G4HPMultiMethod { G4HPMultiPoisson, G4HPMultiBetweenInts };

class G4SaG4nParticleHPProduct
{
   struct toBeCached
   {
...
public:
  ...
  G4SaG4nParticleHPProduct()
  {
	...
	throw G4HadronicException(__FILE__, __LINE__, ("multiplicity method unknown to G4SaG4nParticleHPProduct" + G4String(method)).c_str());
      }
    }
    else
    {
  ...
  inline void Init(std::istream & aDataFile, G4ParticleDefinition* projectile)
  {
    ...
    if( std::getenv("G4PHPTEST") )
      G4cout << " G4SaG4nParticleHPProduct :: Init MassCode "
             << theMassCode << " " << theMass << " theActualStateQValue "
             << theActualStateQValue << G4endl;// GDEB
    if( std::getenv("G4PHPTEST") )
      G4cout << " G4SaG4nParticleHPProduct :: Init theActualStateQValue "
             << theActualStateQValue << G4endl;// GDEB
    ...
    if(theDistLaw==0)
    {
      ...
      theDist = new G4SaG4nParticleHPIsotropic;
    }
    else if(theDistLaw == 1)
    {
      // Continuum energy-angular distribution
      theDist = new G4SaG4nParticleHPContEnergyAngular(projectile);
    }
    else if(theDistLaw == 2)
    {
      // Discrete 2-body scattering
      theDist = new G4SaG4nParticleHPDiscreteTwoBody;
    }
    ...
    {
      // N-Body phase space
      theDist = new G4SaG4nParticleHPNBodyPhaseSpace;
    }
    else if(theDistLaw == 7)
    {
      // Laboratory angular energy paraetrisation
      theDist = new G4SaG4nParticleHPLabAngularEnergy;
    }
    else
    {
      throw G4HadronicException(__FILE__, __LINE__, "distribution law unknown to G4SaG4nParticleHPProduct");
    }
    if(theDist!=0)
    {
  ...
  //G4SaG4nParticleHPEnAngCorrelation
...
private:
  ...
  G4SaG4nParticleHPVector theYield;
