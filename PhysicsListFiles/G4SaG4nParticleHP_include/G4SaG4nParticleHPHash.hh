...
//
#ifndef G4SaG4nParticleHPHash_h
#define G4SaG4nParticleHPHash_h

#include <vector>
#include "globals.hh"
#include "G4SaG4nParticleHPDataPoint.hh"

class G4SaG4nParticleHPHash
{
public:
  G4SaG4nParticleHPHash() 
  {
    theUpper = 0;
    prepared = false;
  }
  ...
  G4SaG4nParticleHPHash(const G4SaG4nParticleHPHash & aHash)
  {
    ...
    if(aHash.theUpper != 0)
    {
      theUpper = new G4SaG4nParticleHPHash(*(aHash.theUpper));
    }
    else
    {
      theUpper = 0;
  ...
  G4SaG4nParticleHPHash & operator = (const G4SaG4nParticleHPHash & aHash)
  {
    if(&aHash != this)
    {
      theIndex = aHash.theIndex;
...
private:
  ...
  G4bool prepared;
  G4SaG4nParticleHPHash * theUpper;
  std::vector<int> theIndex;
  std::vector<G4SaG4nParticleHPDataPoint> theData; // the data
};
#endif
