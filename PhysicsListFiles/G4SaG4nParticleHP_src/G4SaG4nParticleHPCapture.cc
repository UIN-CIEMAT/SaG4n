...
//
#include "G4SaG4nParticleHPCapture.hh"
...
#include "G4Threading.hh"
  ...
  G4SaG4nParticleHPCapture::G4SaG4nParticleHPCapture()
...
//    G4cout <<"Disname="<<dirName<<" numEle="<<numEle<<G4endl;
    //theCapture = new G4SaG4nParticleHPChannel[numEle];
//    G4cout <<"G4SaG4nParticleHPChannel constructed"<<G4endl;
    G4SaG4nParticleHPCaptureFS * theFS = new G4SaG4nParticleHPCaptureFS;
...
//  //    G4cout << "initializing theCapture "<<i<<" "<< numEle<<G4endl;
    ...
    for ( G4int i = 0 ; i < numEle ; i++ ) 
    {
       theCapture.push_back( new G4SaG4nParticleHPChannel );
       (*theCapture[i]).Init((*(G4Element::GetElementTable()))[i], dirName);
       (*theCapture[i]).Register(theFS);
    }
    delete theFS;
//    G4cout << "-------------------------------------------------"<<G4endl;
//    G4cout << "Leaving G4SaG4nParticleHPCapture::G4SaG4nParticleHPCapture"<<G4endl;
*/
  }
  ...
  G4SaG4nParticleHPCapture::~G4SaG4nParticleHPCapture()
  {
    ...
    if ( ! G4Threading::IsWorkerThread() ) {
        if ( theCapture != NULL ) {
            for ( std::vector<G4SaG4nParticleHPChannel*>::iterator
  ...
  G4HadFinalState * G4SaG4nParticleHPCapture::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& aNucleus )
  {
    ...
    if(std::getenv("NeutronHPCapture")) G4cout <<" ####### G4SaG4nParticleHPCapture called"<<G4endl;
...
const std::pair<G4double, G4double> G4SaG4nParticleHPCapture::GetFatalEnergyCheckLevels() const
{
...
/*
void G4SaG4nParticleHPCapture::addChannelForNewElement()
{
   G4SaG4nParticleHPCaptureFS* theFS = new G4SaG4nParticleHPCaptureFS;
   for ( G4int i = numEle ; i < (G4int)G4Element::GetNumberOfElements() ; i++ ) 
   {
      G4cout << "G4SaG4nParticleHPCapture Prepairing Data for the new element of " << (*(G4Element::GetElementTable()))[i]->GetName() << G4endl;
      theCapture.push_back( new G4SaG4nParticleHPChannel );
      (*theCapture[i]).Init((*(G4Element::GetElementTable()))[i], dirName);
      (*theCapture[i]).Register(theFS);
   }
...
G4int G4SaG4nParticleHPCapture::GetVerboseLevel() const
{
   return G4SaG4nParticleHPManager::GetInstance()->GetVerboseLevel();
}
void G4SaG4nParticleHPCapture::SetVerboseLevel( G4int newValue ) 
{
   G4SaG4nParticleHPManager::GetInstance()->SetVerboseLevel(newValue);
}

void G4SaG4nParticleHPCapture::BuildPhysicsTable(const G4ParticleDefinition&)
{
   ...
   if ( G4Threading::IsMasterThread() ) {
      ...
      if ( theCapture == NULL ) theCapture = new std::vector<G4SaG4nParticleHPChannel*>;
...
void G4SaG4nParticleHPCapture::ModelDescription(std::ostream& outFile) const
{
