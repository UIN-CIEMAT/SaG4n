...
#include "G4SaG4nParticleHPThermalScatteringData.hh"
...
G4SaG4nParticleHPThermalScatteringData::G4SaG4nParticleHPThermalScatteringData()
...
// Upper limit of neutron energy 
   ...
   names = new G4SaG4nParticleHPThermalScatteringNames();
}
...
G4bool G4SaG4nParticleHPThermalScatteringData::IsIsoApplicable( const G4DynamicParticle* dp , 
                                                G4int /*Z*/ , G4int /*A*/ ,
...
G4double G4SaG4nParticleHPThermalScatteringData::GetIsoCrossSection( const G4DynamicParticle* dp ,
                                   G4int /*Z*/ , G4int /*A*/ ,
                                   const G4Isotope* /*iso*/  ,
                                   const G4Element* element ,
                                   const G4Material* material )
...
void G4SaG4nParticleHPThermalScatteringData::clearCurrentXSData()
{
   std::map< G4int , std::map< G4double , G4SaG4nParticleHPVector* >* >::iterator it;
   std::map< G4double , G4SaG4nParticleHPVector* >::iterator itt;
...
G4bool G4SaG4nParticleHPThermalScatteringData::IsApplicable( const G4DynamicParticle* aP , const G4Element* anEle )
{
   G4bool result = false;
...
void G4SaG4nParticleHPThermalScatteringData::BuildPhysicsTable(const G4ParticleDefinition& aP)
{
   ...
   if ( G4Threading::IsMasterThread() ) {
      ...
      if ( coherent == NULL ) coherent = new std::map< G4int , std::map< G4double , G4SaG4nParticleHPVector* >* >;
      if ( incoherent == NULL ) incoherent = new std::map< G4int , std::map< G4double , G4SaG4nParticleHPVector* >* >;
      if ( inelastic == NULL ) inelastic = new std::map< G4int , std::map< G4double , G4SaG4nParticleHPVector* >* >;
      ...
      for ( std::map < G4String , G4int >::iterator it = co_dic.begin() ; it != co_dic.end() ; it++ )  
      {
         ...
         std::map< G4double , G4SaG4nParticleHPVector* >*  coh_amapTemp_EnergyCross = readData( full_name );
         coherent->insert ( std::pair < G4int , std::map< G4double , G4SaG4nParticleHPVector* >* > ( ts_ID , coh_amapTemp_EnergyCross ) );

         // Incoherent
         full_name = dirName + "/Incoherent/CrossSection/" + ndl_filename; 
         std::map< G4double , G4SaG4nParticleHPVector* >*  incoh_amapTemp_EnergyCross = readData( full_name );
         incoherent->insert ( std::pair < G4int , std::map< G4double , G4SaG4nParticleHPVector* >* > ( ts_ID , incoh_amapTemp_EnergyCross ) );

         // Inelastic
         full_name = dirName + "/Inelastic/CrossSection/" + ndl_filename; 
         std::map< G4double , G4SaG4nParticleHPVector* >*  inela_amapTemp_EnergyCross = readData( full_name );
         inelastic->insert ( std::pair < G4int , std::map< G4double , G4SaG4nParticleHPVector* >* > ( ts_ID , inela_amapTemp_EnergyCross ) );
...
std::map< G4double , G4SaG4nParticleHPVector* >* G4SaG4nParticleHPThermalScatteringData::readData ( G4String full_name ) 
{

   std::map< G4double , G4SaG4nParticleHPVector* >*  aData = new std::map< G4double , G4SaG4nParticleHPVector* >; 
   
   //std::ifstream theChannel( full_name.c_str() );
   std::istringstream theChannel;
   G4SaG4nParticleHPManager::GetInstance()->GetDataStream(full_name,theChannel);

   //G4cout << "G4SaG4nParticleHPThermalScatteringData " << name << G4endl;
   ...
   while ( theChannel >> dummy )   // MF // Loop checking, 11.05.2015, T. Koi
   {
      ...
      G4double temp; 
      theChannel >> temp;   
      G4SaG4nParticleHPVector* anEnergyCross = new G4SaG4nParticleHPVector;
      G4int nData;
      theChannel >> nData;
      anEnergyCross->Init ( theChannel , nData , eV , barn );
      aData->insert ( std::pair < G4double , G4SaG4nParticleHPVector* > ( temp , anEnergyCross ) );
   }
   //theChannel.close();
...
void G4SaG4nParticleHPThermalScatteringData::DumpPhysicsTable( const G4ParticleDefinition& aP )
{
   if( &aP != G4Neutron::Neutron() ) 
     throw G4HadronicException(__FILE__, __LINE__, "Attempt to use NeutronHP data for particles other than neutrons!!!");  
//  G4cout << "G4SaG4nParticleHPThermalScatteringData::DumpPhysicsTable still to be implemented"<<G4endl;
}
...
/*
G4double G4SaG4nParticleHPThermalScatteringData::GetCrossSection( const G4DynamicParticle* aP , const G4Element*anE , G4double aT )
{
   ...
   //G4cout << "G4SaG4nParticleHPThermalScatteringData::GetCrossSection  Tot= " << result/barn << " Coherent= " << Xcoh/barn << " Incoherent= " << Xincoh/barn << " Inelastic= " << Xinela/barn << G4endl;
...
G4double G4SaG4nParticleHPThermalScatteringData::GetCrossSection( const G4DynamicParticle* aP , const G4Element*anE , const G4Material* aM )
{
   G4double result = 0;
...
G4double G4SaG4nParticleHPThermalScatteringData::GetInelasticCrossSection( const G4DynamicParticle* aP , const G4Element*anE , const G4Material* aM )
{
   G4double result = 0;
   G4int ts_id = getTS_ID( aM , anE );
   G4double aT = aM->GetTemperature();
...
G4double G4SaG4nParticleHPThermalScatteringData::GetCoherentCrossSection( const G4DynamicParticle* aP , const G4Element*anE , const G4Material* aM )
{
   G4double result = 0;
   G4int ts_id = getTS_ID( aM , anE );
   G4double aT = aM->GetTemperature();
...
G4double G4SaG4nParticleHPThermalScatteringData::GetIncoherentCrossSection( const G4DynamicParticle* aP , const G4Element*anE , const G4Material* aM )
{
   G4double result = 0;
   G4int ts_id = getTS_ID( aM , anE );
   G4double aT = aM->GetTemperature();
...
G4int G4SaG4nParticleHPThermalScatteringData::getTS_ID ( const G4Material* material , const G4Element* element )
{
   G4int result = -1;
   if ( dic.find( std::pair < const G4Material* , const G4Element* > ( (G4Material*)NULL , element ) ) != dic.end() ) 
...
G4double G4SaG4nParticleHPThermalScatteringData::GetX ( const G4DynamicParticle* aP, G4double aT , std::map < G4double , G4SaG4nParticleHPVector* >* amapTemp_EnergyCross )
{
   ...
   if ( amapTemp_EnergyCross->size() == 1 ) { 
      if ( std::fabs ( aT - amapTemp_EnergyCross->begin()->first ) / amapTemp_EnergyCross->begin()->first > 0.1 ) {
         G4cout << "G4SaG4nParticleHPThermalScatteringData:: The temperature of material (" 
   ...
   std::map< G4double , G4SaG4nParticleHPVector* >::iterator it; 
   ...
   //G4cout << "G4SaG4nParticleHPThermalScatteringData::GetX TH " << TH << " E " << eKinetic <<  " XH " << XH << G4endl;

   if ( it != amapTemp_EnergyCross->begin() ) it--;
   G4double TL = it->first;
   G4double XL = it->second->GetXsec ( eKinetic ); 

   //G4cout << "G4SaG4nParticleHPThermalScatteringData::GetX TL " << TL << " E " << eKinetic <<  " XL " << XL << G4endl;
...
void G4SaG4nParticleHPThermalScatteringData::AddUserThermalScatteringFile( G4String nameG4Element , G4String filename )
{
   names->AddThermalElement( nameG4Element , filename );
}
void G4SaG4nParticleHPThermalScatteringData::CrossSectionDescription(std::ostream& outFile) const
{
