...
#include "G4SaG4nParticleHPContAngularPar.hh"
...
G4SaG4nParticleHPContAngularPar::G4SaG4nParticleHPContAngularPar( G4ParticleDefinition* projectile )
{  
  theAngular = 0;
  ...
  adjustResult = true;
  if ( G4SaG4nParticleHPManager::GetInstance()->GetDoNotAdjustFinalState() ) adjustResult = false;
  ...
  void G4SaG4nParticleHPContAngularPar::Init(std::istream & aDataFile, G4ParticleDefinition* projectile)
  { 
    adjustResult = true;
    if ( G4SaG4nParticleHPManager::GetInstance()->GetDoNotAdjustFinalState() ) adjustResult = false;
    ...
    theEnergy *= eV;
    theAngular = new G4SaG4nParticleHPList [nEnergies];
  ...
  G4ReactionProduct * 
  G4SaG4nParticleHPContAngularPar::Sample(G4double anEnergy, G4double massCode, G4double /*targetMass*/, 
                                    G4int angularRep, G4int /*interpolE*/ )
  {
    // The following line is needed because it may change between runs by UI command
    if ( G4SaG4nParticleHPManager::GetInstance()->GetDoNotAdjustFinalState() ) adjustResult = false;
 
    if( std::getenv("G4PHPTEST") ) G4cout << "  G4SaG4nParticleHPContAngularPar::Sample " << anEnergy << " " << massCode << " " << angularRep << G4endl; //GDEB
    if ( fCache.Get() == 0 ) cacheInit();
    ...
    else if(A==4)
    {
      ...
      if(Z!=2) throw G4HadronicException(__FILE__, __LINE__, "G4SaG4nParticleHPContAngularPar: Unknown ion case 1");    
    }
    else
    {
            ...
            if ( theAngular[j].GetLabel() == 0.0 ) {
               ...
               if ( j != nEnergies-1 ) {
                  ...
                  if ( theAngular[j].GetValue(0) != 0.0 ) {
                     throw G4HadronicException(__FILE__, __LINE__, "G4SaG4nParticleHPContAngularPar: Unexpected non zero value of theAngular[nEnergies-1].GetValue(0)");    
                  }
               }
            }
...
//          Discrete Emission 
            for ( G4int j = 0 ; j < nDiscreteEnergies ; j++ )
	    {
 	    ...
 	    G4SaG4nParticleHPLegendreStore theStore(1);
...
//          Continuous Emission
            for ( G4int j = nDiscreteEnergies ; j < nEnergies ; j++ )
	    {
	       ...
	       if ( random < running[ j ] ) 
            ...
            G4SaG4nParticleHPLegendreStore theStore(2);
            ...
            for (G4int j=0;j<nAngularParameters;j++)
            {
               ...
               if ( it == 0 ) 
               {
                  ...
                  //G4cout << "110611 G4SaG4nParticleHPContAngularPar::Sample it = 0; invetigation required " << G4endl;
                  itt = it+1; 
               }
...
*/
    ...
    else if(angularRep==2)
    {
      ...
      running[0]=0;
      G4double weighted = 0;
      if( std::getenv("G4PHPTEST") ) G4cout << "  G4SaG4nParticleHPContAngularPar::Sample nEnergies " << nEnergies << G4endl;
      for(j=1; j<nEnergies; j++)
      {
        ...
        weighted += theInt.GetWeightedBinIntegral(theManager.GetScheme(j-1),
                             theAngular[j-1].GetLabel(), theAngular[j].GetLabel(),
                             theAngular[j-1].GetValue(0), theAngular[j].GetValue(0));
	if( std::getenv("G4PHPTEST") ) G4cout << "  G4SaG4nParticleHPContAngularPar::Sample " << j << " running " << running[j] 
      ...
      G4SaG4nParticleHPKallbachMannSyst theKallbach(compoundFraction,
      ...
      if( std::getenv("G4PHPTEST") ) G4cout << " G4SaG4nParticleHPKallbachMannSyst::Sample resulttest " << cosTh << G4endl; //GDEB
    }
    else if(angularRep>10&&angularRep<16)
    {
      ...
      if(it<nDiscreteEnergies||it==0) 
      {
        if(it==0)
        {
          ...
          G4SaG4nParticleHPVector theStore; 
	  G4int aCounter = 0;
      ...
      else
      {
        ...
        fsEnergy = theInt.Interpolate(theManager.GetInverseScheme(it),
                                      random,x1,x2,y1,y2);
        G4SaG4nParticleHPVector theBuff1;
        G4SaG4nParticleHPVector theBuff2;
        G4InterpolationManager aMan;
        aMan.Init(angularRep-10, nAngularParameters-1);
...
//      Bug Report #1366 from L. Russell 
    ...
    {
      throw G4HadronicException(__FILE__, __LINE__, "G4SaG4nParticleHPContAngularPar::Sample: Unknown angular representation");
    }
...
void G4SaG4nParticleHPContAngularPar::PrepareTableInterpolation(const G4SaG4nParticleHPContAngularPar* angParPrev)
{
  ...
  for(ie=nDiscreteEnergies; ie<nEnergies; ie++) {
    ...
    //-    if( getenv("G4PHPTEST2") ) G4cout <<this << " G4SaG4nParticleHPContAngularPar::PrepareTableInterpolation  theEnergiesTransformed1 " << enerT << G4endl; //GDEB
  } 
  ...
  for(ie=nDiscreteEnergiesPrev; ie<nEnergiesPrev; ie++) {
    ...
    theEnergiesTransformed.insert(enerT);
    //-    if( getenv("G4PHPTEST2") ) G4cout << this << " G4SaG4nParticleHPContAngularPar::PrepareTableInterpolation  theEnergiesTransformed2 " << enerT << G4endl; //GDEB
  }
...
void G4SaG4nParticleHPContAngularPar::BuildByInterpolation(G4double anEnergy, G4InterpolationScheme aScheme, 
							G4SaG4nParticleHPContAngularPar & angpar1, 
							G4SaG4nParticleHPContAngularPar & angpar2) 
{
  ...
  for( itede = theDiscreteEnergies.begin(); itede != theDiscreteEnergies.end(); itede++, ie++ ) {
	...
	val2 = 0.;
      ...
      G4double value = theInt.Interpolate(aScheme, anEnergy, 
					  ...
					  val1,
					  val2);
      if( std::getenv("G4PHPTEST2") ) G4cout << ie << " " << ip << " G4SaG4nParticleHPContAngularPar::Merge DiscreteEnergies  val1 " << val1 << " val2 " << val2 << " value " << value << G4endl; //GDEB
  ...
  if( std::getenv("G4PHPTEST2") )  G4cout << " G4SaG4nParticleHPContAngularPar::Merge E " << anEnergy << " minmax " << theMinEner << " " << theMaxEner << G4endl; //GDEB
  ...
  for(ie=nDiscreteEnergies; ie<nEnergies; ie++,iteet++) { 
    ...
    if( std::getenv("G4PHPTEST2") )  G4cout << ie << " " << ie1 << " " << ie2 << " G4SaG4nParticleHPContAngularPar::loop eT " << eT << " -> eN " << eN << " e1 " << e1 << " e2 " << e2 << G4endl; //GDEB
    ...
    for(G4int ip=0; ip<nAngularParameters; ip++) {
      ...
      if ( theMaxEner != theMinEner ) {
         ...
         throw G4HadronicException(__FILE__, __LINE__, "G4SaG4nParticleHPContAngularPar::PrepareTableInterpolation theMaxEner == theMinEner and  value != 0.");
      }
      if( std::getenv("G4PHPTEST2") ) G4cout << ie << " " << ip << " G4SaG4nParticleHPContAngularPar::Merge val1 " << val1 << " val2 " << val2 << " value " << value << G4endl; //GDEB
      //-      val1 = angpar1.theAngular[ie1-1].GetValue(ip) * (maxEner1-minEner1); 
      //-      val2 = angpar2.theAngular[ie2-1].GetValue(ip) * (maxEner2-minEner2); 
      //-      if( getenv("G4PHPTEST2") ) G4cout << ie << " " << ip << " G4SaG4nParticleHPContAngularPar::MergeOLD val1 " << val1 << " val2 " << val2 << " value " << value << G4endl; //GDEB
  ...
  if( std::getenv("G4PHPTEST2") ) {
    G4cout << " G4SaG4nParticleHPContAngularPar::Merge ANGPAR1 " << G4endl; //GDEB
    angpar1.Dump();
    G4cout << " G4SaG4nParticleHPContAngularPar::Merge ANGPAR2 " << G4endl;
    angpar2.Dump();
    G4cout << " G4SaG4nParticleHPContAngularPar::Merge ANGPARNEW " << G4endl;
    Dump();
  }   
}

void G4SaG4nParticleHPContAngularPar::Dump()
{
