...
//G4FFG_FUNCTIONENTER__
    ...
    if(newIsotope.second || newIsotope.first->second == NULL)
    {
        // Get the data file
        G4bool flag;
        G4SaG4nParticleHPDataUsed dataFile = fileNames.GetName(A, Z, M, dataDirectory, "FF", flag);
        ...
        G4SaG4nParticleHPManager::GetInstance()->GetDataStream(dataFileName, dataStream);
        if(!dataStream)
        {
            //G4FFG_FUNCTIONLEAVE__
