...
//
#include "G4SaG4nParticleHPContEnergyAngular.hh"

G4ReactionProduct * G4SaG4nParticleHPContEnergyAngular::Sample(G4double anEnergy, G4double massCode, G4double /*mass*/)
{
   ...
   if( std::getenv("G4PHPTEST") )    G4cout << i << " G4SaG4nParticleHPContEnergyAngular dataE " << theAngular[i].GetEnergy() << " > " << anEnergy << " it_theAngular " << it << " interpolation " << theInterpolation << G4endl; //GDEB
   ...
   else
   {
     ...
     //--- create new 
     //     if( theManager.GetScheme(0) != LINLIN ) {  // asserted in G4SaG4nParticleHPContEnergyAngular::init there is only one range
#ifdef PHP_AS_HP
...
#else
    if( std::getenv("G4PHPTEST") )     G4cout << i << " G4SaG4nParticleHPContEnergyAngular To BUILDBYINTERPOLATION " << it << " : " << theAngular[it].GetEnergy()<< " , " << theAngular[it].GetNEnergies() << " " << it-1 << " : " << theAngular[it-1].GetEnergy()<< " : " << theAngular[it-1].GetNEnergies() << G4endl; //GDEB

//     G4SaG4nParticleHPContAngularPar * fAngular = new  G4SaG4nParticleHPContAngularPar(theProjectile ); //fix start
     if (fCacheAngular.Get() == NULL) {
		G4SaG4nParticleHPContAngularPar* angpar = new G4SaG4nParticleHPContAngularPar(theProjectile);
...
//     delete fAngular; //fix end
   ...
   //      G4cout << " 0 0 @@@ G4SaG4nParticleHPContEnergyAngular::Sample " << result->GetDefinition()->GetParticleName() << " E= " << result->GetKineticEnergy() << G4endl;//GDEB
   return result;
}

G4double G4SaG4nParticleHPContEnergyAngular::
MeanEnergyOfThisInteraction()
{
   G4double result(0);
   if(currentMeanEnergy.Get()<-1)
   {
     throw G4HadronicException(__FILE__, __LINE__, "G4SaG4nParticleHPContEnergyAngular: Logical error in Product class");
   }
   else
   {
...
void G4SaG4nParticleHPContEnergyAngular::ClearHistories()
{ 
   if ( theAngular!= NULL )
   { 
