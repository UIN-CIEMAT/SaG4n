...
void G4SaG4nParticleHPFinalState::adjust_final_state ( G4LorentzVector init_4p_lab )
{
   ...
   if ( G4SaG4nParticleHPManager::GetInstance()->GetDoNotAdjustFinalState() ) return;
   ...
   {
      //G4cout << "G4SaG4nParticleHPFinalState::adjust_final_state theResult.Get()->GetSecondary( i )->GetParticle()->GetDefinition()->GetParticleName() = " << theResult.Get()->GetSecondary( i )->GetParticle()->GetDefinition()->GetParticleName() << G4endl;
   ...
   else
   {
	...
	resi_pd = G4IonTable::GetIonTable()->GetIon ( int(baseZNew - sum_Z) , (int)(baseANew - sum_A) , 0.0 );
      ...
      if ( needOneMoreSec )
      {
	   ...
	   if( !oneMoreSec_pd ) {
	     G4cerr << this << "G4SaG4nParticleHPFinalState oneMoreSec_pd Z " << baseZNew << " - " << sum_Z << " A " << baseANew << " - " << sum_A << " projectile " << theProjectile->GetParticleName() << G4endl;
	     G4Exception("G4SaG4nParticleHPFinalState:adjust_final_state",
			 "Warning",
			 JustWarning,
	...
	if( theProjectile == G4Neutron::Neutron() ) {
	  ...
	  ndlANew += 4;
	   ...
	   if( !resi_pd ) {
	     G4cerr << "G4SaG4nParticleHPFinalState resi_pd Z " << max_SecZ << " - " << dif_Z << " A " << max_SecA << " - " << dif_A << " projectile " << theProjectile->GetParticleName() << G4endl;
	     G4Exception("G4SaG4nParticleHPFinalState:adjust_final_state",
			 "Warning",
			 JustWarning,
			 "No adjustment will be done!");
