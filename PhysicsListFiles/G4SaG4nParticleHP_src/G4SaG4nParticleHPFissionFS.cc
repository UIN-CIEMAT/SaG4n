...
#include "G4PhysicsModelCatalog.hh"
 ...
 G4SaG4nParticleHPFissionFS::G4SaG4nParticleHPFissionFS()
 {
 ...
 void G4SaG4nParticleHPFissionFS::Init (G4double A, G4double Z, G4int M, G4String & dirName, G4String & aFSType, G4ParticleDefinition* projectile )
 {
    //G4cout << "G4SaG4nParticleHPFissionFS::Init " << A << " " << Z << " " << M << G4endl;
    ...
    if ( G4SaG4nParticleHPManager::GetInstance()->GetProduceFissionFragments() && theFF.HasFSData() ) 
    {
 ...
 G4HadFinalState * G4SaG4nParticleHPFissionFS::ApplyYourself(const G4HadProjectile & theTrack)
 {  

    //Because it may change by UI command 
    produceFissionFragments=G4SaG4nParticleHPManager::GetInstance()->GetProduceFissionFragments(); 

 //G4cout << "G4SaG4nParticleHPFissionFS::ApplyYourself " << G4endl;
// prepare neutron
...
//G4cout << "delayed" << G4endl;
     for(i0=Prompt; i0<Prompt+delayed; i0++)
     {
       ...
       if ( theDecayConstants[i0-Prompt] > 1.0e-30 ) {
         ...
         G4Exception( "G4SaG4nParticleHPFissionFS::ApplyYourself ", "HAD_FISSIONHP_001", JustWarning, ed );
       }
...
//    G4cout <<"Number of delayed = "<<delayed<<G4endl;
   ...
   G4SaG4nParticleHPFissionERelease * theERelease = theFS.GetEnergyRelease();
