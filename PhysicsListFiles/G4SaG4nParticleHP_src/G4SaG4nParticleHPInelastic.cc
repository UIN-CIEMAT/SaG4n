...
//    delete [] theInelastic;
  ...
  G4HadFinalState * G4SaG4nParticleHPInelastic::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& aNucleus )
  {
	...
	if( std::getenv("G4SaG4nParticleHPDebug") && G4HadronicParameters::Instance()->GetVerboseLevel() > 0 )
	  G4cout << " G4SaG4nParticleHPInelastic XSEC ELEM " << i << " = " << xSec[i] << G4endl;
	#endif
#endif
    ...
    #ifdef G4VERBOSE
    if( std::getenv("G4SaG4nParticleHPDebug") && G4HadronicParameters::Instance()->GetVerboseLevel() > 0 )
      G4cout << " G4SaG4nParticleHPInelastic SELECTED ELEM " << it << " = " << theMaterial->GetElement(it)->GetName() << " FROM MATERIAL " << theMaterial->GetName() << G4endl;
    #endif
#endif
