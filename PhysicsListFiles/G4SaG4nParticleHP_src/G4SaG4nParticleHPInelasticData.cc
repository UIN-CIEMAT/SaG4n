...
G4SaG4nParticleHPInelasticData::G4SaG4nParticleHPInelasticData(G4ParticleDefinition* projectile)
  : G4VCrossSectionDataSet("")
{
  ...
  if( projectile == G4Neutron::Neutron() ) {
    ...
    G4String message("G4SaG4nParticleHPInelasticData may only be called for neutron, proton, deuteron, triton, He3 or alpha, while it is called for " + projectile->GetParticleName());
    throw G4HadronicException(__FILE__, __LINE__,message.c_str());
  }
  //  G4cout << this << " G4SaG4nParticleHPInelasticData::G4SaG4nParticleHPInelasticData " << projectile->GetParticleName() << " DATADIR " << dataDirVariable << G4endl;//GDEB
  ...
  if ( G4HadronicParameters::Instance()->GetVerboseLevel() > 0 ) {
    G4cout << "@@@ G4SaG4nParticleHPInelasticData instantiated for particle " << projectile->GetParticleName() << " data directory variable is " << dataDirVariable << " pointing to " << dirName << G4endl;
  }
  #endif
  ...
  if ( G4Threading::IsMasterThread() ) {
    theHPData = new G4SaG4nParticleHPData( theProjectile ); 
  } else {
...
G4SaG4nParticleHPInelasticData::~G4SaG4nParticleHPInelasticData()
{
...
G4bool G4SaG4nParticleHPInelasticData::IsIsoApplicable( const G4DynamicParticle* dp , 
                                                G4int /*Z*/ , G4int /*A*/ ,
                                                const G4Element* /*elm*/ ,
...
G4double G4SaG4nParticleHPInelasticData::GetIsoCrossSection( const G4DynamicParticle* dp ,
                                   G4int /*Z*/ , G4int /*A*/ ,
                                   const G4Isotope* /*iso*/  ,
...
/*
G4bool G4SaG4nParticleHPInelasticData::IsApplicable(const G4DynamicParticle*aP, const G4Element*)
{
  G4bool result = true;
...
//void G4SaG4nParticleHPInelasticData::BuildPhysicsTableHP(G4ParticleDefinition* projectile,const char* /* dataDirVariable */)
void G4SaG4nParticleHPInelasticData::BuildPhysicsTable( const G4ParticleDefinition& projectile )
{
  //  if(&projectile!=G4Neutron::Neutron()) 
  //     throw G4HadronicException(__FILE__, __LINE__, "Attempt to use NeutronHP data for particles other than neutrons!!!");  

   if ( G4Threading::IsWorkerThread() ) {
      theCrossSections = G4SaG4nParticleHPManager::GetInstance()->GetInelasticCrossSections( &projectile );
      return;
   } else {
      if ( theHPData == NULL ) theHPData = G4SaG4nParticleHPData::Instance( const_cast<G4ParticleDefinition*> ( &projectile ) ); 
   }
...
// TKDB
  ...
  //G4SaG4nParticleHPData* hpData = new G4SaG4nParticleHPData(projectile); //NEW
  ...
  {
    //NEW    G4PhysicsVector* physVec = G4SaG4nParticleHPData::
   ...
   G4SaG4nParticleHPManager::GetInstance()->RegisterInelasticCrossSections( &projectile , theCrossSections );
}

void G4SaG4nParticleHPInelasticData::DumpPhysicsTable(const G4ParticleDefinition& projectile)
{
...
//
  ...
  //G4cout << "G4SaG4nParticleHPInelasticData::DumpPhysicsTable still to be implemented"<<G4endl;
  #endif
}

#include "G4NucleiProperties.hh"

G4double G4SaG4nParticleHPInelasticData::
GetCrossSection(const G4DynamicParticle* projectile, const G4Element*anE, G4double aT)
{
  ...
  //  G4cout << " G4SaG4nParticleHPInelasticData 2 " << size << G4endl;//GDEB
  ...
  while(counter == 0 || std::abs(buffer-result/std::max(1,counter)) > 0.01*buffer) // Loop checking, 11.05.2015, T. Koi
  {
    ...
    while (counter<size) // Loop checking, 11.05.2015, T. Koi
    {
      ...
      //       G4cout << " G4SaG4nParticleHPInelasticData aXsection " << aXsection << " index " << index << " theEkin " << theEkin << " outOfRange " << outOfRange <<G4endl;//GDEB
     if(aXsection <0) 
      {
        if(failCount<1000)
	{
...
*/
//   G4cout << this << " G4SaG4nParticleHPInelasticData result " << result << G4endl; //GDEB

  return result;
}

G4int G4SaG4nParticleHPInelasticData::GetVerboseLevel() const 
{
   return G4SaG4nParticleHPManager::GetInstance()->GetVerboseLevel();
}
void G4SaG4nParticleHPInelasticData::SetVerboseLevel( G4int newValue ) 
{
   G4SaG4nParticleHPManager::GetInstance()->SetVerboseLevel(newValue);
}
void G4SaG4nParticleHPInelasticData::CrossSectionDescription(std::ostream& outFile) const
{
