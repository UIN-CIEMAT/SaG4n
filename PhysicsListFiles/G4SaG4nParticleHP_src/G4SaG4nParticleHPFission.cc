...
//
#include "G4SaG4nParticleHPFission.hh"
#include "G4SaG4nParticleHPFissionFS.hh"
#include "G4SystemOfUnits.hh"
#include "G4SaG4nParticleHPManager.hh"
#include "G4Threading.hh"

  G4SaG4nParticleHPFission::G4SaG4nParticleHPFission()
...
/*
    ...
    //theFission = new G4SaG4nParticleHPChannel[numEle];
    ...
    for ( G4int i = 0 ; i < numEle ; i++ ) 
    {
      theFission.push_back( new G4SaG4nParticleHPChannel );
      if((*(G4Element::GetElementTable()))[i]->GetZ()>87) //TK modified for ENDF-VII
      {
       (*theFission[i]).Init((*(G4Element::GetElementTable()))[i], dirName);
       (*theFission[i]).Register(&theFS);
...
*/
  ...
  G4HadFinalState * G4SaG4nParticleHPFission::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& aNucleus )
  {
...
const std::pair<G4double, G4double> G4SaG4nParticleHPFission::GetFatalEnergyCheckLevels() const
{
...
/*
void G4SaG4nParticleHPFission::addChannelForNewElement()
{
   for ( G4int i = numEle ; i < (G4int)G4Element::GetNumberOfElements() ; i++ ) 
   {
      ...
      if ( (*(G4Element::GetElementTable()))[i]->GetZ() > 87 ) //TK modified for ENDF-VII
      {
         G4cout << "G4SaG4nParticleHPFission Prepairing Data for the new element of " << (*(G4Element::GetElementTable()))[i]->GetName() << G4endl;
         (*theFission[i]).Init((*(G4Element::GetElementTable()))[i], dirName);
         (*theFission[i]).Register(&theFS);
      }
   }
...
G4int G4SaG4nParticleHPFission::GetVerboseLevel() const
{
   return G4SaG4nParticleHPManager::GetInstance()->GetVerboseLevel();
}
void G4SaG4nParticleHPFission::SetVerboseLevel( G4int newValue ) 
{
   G4SaG4nParticleHPManager::GetInstance()->SetVerboseLevel(newValue);
}
void G4SaG4nParticleHPFission::BuildPhysicsTable(const G4ParticleDefinition&)
{
   ...
   if ( G4Threading::IsMasterThread() ) {
      ...
      if ( theFission == NULL ) theFission = new std::vector<G4SaG4nParticleHPChannel*>;
      ...
      for ( G4int i = numEle ; i < (G4int)G4Element::GetNumberOfElements() ; i++ ) {
         ...
         if ((*(G4Element::GetElementTable()))[i]->GetZ()>87) { //TK modified for ENDF-VII
            ((*theFission)[i])->Init((*(G4Element::GetElementTable()))[i], dirName);
            ((*theFission)[i])->Register( new G4SaG4nParticleHPFissionFS );
         }
      }
...
void G4SaG4nParticleHPFission::ModelDescription(std::ostream& outFile) const
{
