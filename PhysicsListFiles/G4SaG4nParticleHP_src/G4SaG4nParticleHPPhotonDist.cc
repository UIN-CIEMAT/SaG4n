...
G4bool G4SaG4nParticleHPPhotonDist::InitMean(std::istream & aDataFile)
{
  ...
  if(aDataFile >> repFlag)
  {
    ...
    // multiplicities
      ...
      theYield = new G4SaG4nParticleHPVector[nDiscrete];
    ...
    else if(repFlag == 2)
    {
	 ...
	 {
           throw G4HadronicException(__FILE__, __LINE__, "G4SaG4nParticleHPPhotonDist: Unknown conversion flag");
	 }
      }
       // Note, that this is equivalent to using the 'Gamma' classes.
      // throw G4HadronicException(__FILE__, __LINE__, "G4SaG4nParticleHPPhotonDist: Transition probability array not sampled for the moment.");
    }
    else
    {
      G4cout << "Data representation in G4SaG4nParticleHPPhotonDist: "<<repFlag<<G4endl;
      throw G4HadronicException(__FILE__, __LINE__, "G4SaG4nParticleHPPhotonDist: This data representation is not implemented.");
    }
  }
  else
  {
...
void G4SaG4nParticleHPPhotonDist::InitAngular(std::istream & aDataFile)
{
  G4int i, ii;
  ...
  if (isoFlag != 1)
  {
    if (repFlag == 2) G4cout << "G4SaG4nParticleHPPhotonDist: repFlag == 2 && isoFlag != 1 is unexpected! If you use G4ND3.x, then please report to Geant4 HyperNews. " << G4endl;
    aDataFile >> tabulationType >> nDiscrete2 >> nIso;
//080731
      if (theGammas != NULL && nDiscrete2 != nDiscrete) 
        G4cout << "080731c G4SaG4nParticleHPPhotonDist nDiscrete2 != nDiscrete, It looks like something wrong in your NDL files. Please update the latest. If you still have this messages after the update, then please report to Geant4 Hyper News." << G4endl;
      ...
      std::vector < G4SaG4nParticleHPVector* > vct_pXS_par;
      if ( theGammas != NULL && theShells != NULL ) 
      {
         ...
         for ( i = 0 ; i < nDiscrete ; i++ )
         {
            ...
            G4SaG4nParticleHPVector* hpv = new G4SaG4nParticleHPVector;
...
//080731
    ...
    if(tabulationType==1)theLegendre=new G4SaG4nParticleHPLegendreTable *[nDiscrete2-nIso];
    if(tabulationType==2)theAngular =new G4SaG4nParticleHPAngularP *[nDiscrete2-nIso];
    for(i=nIso; i< nDiscrete2; i++)
    {
      if(tabulationType==1) 
      {
        ...
        theGammas[i]*=eV;
        theShells[i]*=eV;
        theLegendre[i-nIso]=new G4SaG4nParticleHPLegendreTable[nNeu[i-nIso]];
      ...
      else if(tabulationType==2)
      {
        ...
        theGammas[i]*=eV;
        theShells[i]*=eV;
        theAngular[i-nIso]=new G4SaG4nParticleHPAngularP[nNeu[i-nIso]];
        for (ii=0; ii<nNeu[i-nIso]; ii++)
        {
...
//080731
     if ( vct_gammas_par.size() > 0 ) 
     {
        ...
        //Garbage collection 
        for ( std::vector < G4SaG4nParticleHPVector* >::iterator 
...
void G4SaG4nParticleHPPhotonDist::InitEnergies(std::istream & aDataFile)
{
  ...
  probs = new G4SaG4nParticleHPVector[nPartials];
  partials = new G4SaG4nParticleHPPartial * [nPartials];
  G4int nen;
  G4int dummy;
  for (i=0; i<nPartials; i++)
  {
    ...
    aDataFile >> nen;
    partials[i] = new G4SaG4nParticleHPPartial(nen);
    partials[i]->InitInterpolation(aDataFile);
    partials[i]->Init(aDataFile);
  }
}

void G4SaG4nParticleHPPhotonDist::InitPartials(std::istream& aDataFile,
                                          G4SaG4nParticleHPVector* theXsec)
{
  ...
  disType = new G4int[nDiscrete];
  thePartialXsec = new G4SaG4nParticleHPVector[nDiscrete];
  for(i=0; i<nDiscrete; i++)
  {
  ...
  //G4cout << "G4SaG4nParticleHPPhotonDist::InitPartials Test " << G4endl;
  //G4cout << "G4SaG4nParticleHPPhotonDist::InitPartials nDiscrete " << nDiscrete << G4endl;
  //G4SaG4nParticleHPVector* aHP = new G4SaG4nParticleHPVector;
  //aHP->Check(1);
}

G4ReactionProductVector * G4SaG4nParticleHPPhotonDist::GetPhotons(G4double anEnergy)
{

  //G4cout << "G4SaG4nParticleHPPhotonDist::GetPhotons repFlag " << repFlag << G4endl;
  ...
  if (repFlag==1) {
    ...
    if (nDiscrete == 1 && nPartials == 1) {
      if (actualMult.Get()->at(0) > 0) {
        ...
        count++;
	if (count > nSecondaries) throw G4HadronicException(__FILE__, __LINE__, "G4SaG4nParticleHPPhotonDist::GetPhotons inconsistency");
      }
  ...
  //      G4cout << "Isotropic distribution in PhotonDist"<<temp<<G4endl;
	...
	else if(tabulationType==1)
	{
	    ...
	    if(theLegendre[ii-nIso][iii].GetEnergy()>anEnergy)
          ...
          G4SaG4nParticleHPLegendreStore aStore(2);
