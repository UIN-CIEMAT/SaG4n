...
G4ThreadLocal G4int G4SaG4nParticleHPChannelList::trycounter = 0;

 G4SaG4nParticleHPChannelList::G4SaG4nParticleHPChannelList(G4int n, G4ParticleDefinition* projectile)
  :theProjectile(projectile)
  { 
    nChannels = n;
    theChannels  = new G4SaG4nParticleHPChannel * [n];
...
#include "G4Neutron.hh"
  G4SaG4nParticleHPChannelList::G4SaG4nParticleHPChannelList()
  {
    nChannels = 0;
    theChannels = 0;
    allChannelsCreated = false;
  ...
  G4HadFinalState * G4SaG4nParticleHPChannelList::ApplyYourself(const G4Element * , const G4HadProjectile & aTrack)
  {
    ...
    for(i=0; i<nChannels; i++)
    {
      ...
      if(theChannels[i]->HasAnyData(isotope))
      {
        ...
        targA=(G4int)theChannels[i]->GetN(isotope); //Will be simply used the last valid value
        targZ=(G4int)theChannels[i]->GetZ(isotope);
	//	G4cout << " G4SaG4nParticleHPChannelList " << i << " targA " << targA << " targZ " << targZ << " running " << running[i] << G4endl;
      }
    }
    ...
    if ( running[nChannels-1] == 0 )
    {
       ...
       G4SaG4nParticleHPManager::GetInstance()->GetReactionWhiteBoard()->SetTargZ( targZ ); 
       delete [] running;
       return &unChanged;
    }
    //TK120607
...
void G4SaG4nParticleHPChannelList::Init(G4Element * anElement, const G4String & dirName, G4ParticleDefinition* projectile )
  {
    theDir = dirName;
...
//    G4cout << theElement << G4endl;
  ...
  void G4SaG4nParticleHPChannelList::Register(G4SaG4nParticleHPFinalState * theFS, 
					 const G4String & aName )
  {
    if(!allChannelsCreated)
    {
      if(nChannels!=0)
      {
	G4SaG4nParticleHPChannel ** theBuffer = new G4SaG4nParticleHPChannel * [nChannels+1];
	G4int i;
	for(i=0; i<nChannels; i++)
	{
	...
	theChannels = theBuffer;
      ...
      {
	theChannels = new G4SaG4nParticleHPChannel * [nChannels+1];
      }
      G4String name;
      name = aName+"/";
      theChannels[nChannels] = new G4SaG4nParticleHPChannel(theProjectile);
...
void G4SaG4nParticleHPChannelList::DumpInfo(){
