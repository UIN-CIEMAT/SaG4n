...
//
#include "G4SaG4nParticleHPThermalScattering.hh"
#include "G4SaG4nParticleHPThermalScatteringData.hh"
#include "G4SaG4nParticleHPThermalScatteringNames.hh"
#include "G4SaG4nParticleHPElastic.hh"
...
G4SaG4nParticleHPThermalScattering::G4SaG4nParticleHPThermalScattering()
                             :G4HadronicInteraction("NeutronHPThermalScattering")
,coherentFSs(nullptr)
,incoherentFSs(nullptr)
,inelasticFSs(nullptr)
{
   theHPElastic = new G4SaG4nParticleHPElastic();

   SetMinEnergy( 0.*eV );
   SetMaxEnergy( 4*eV );
   theXSection = new G4SaG4nParticleHPThermalScatteringData();
...
void G4SaG4nParticleHPThermalScattering::clearCurrentFSData() {
...
void G4SaG4nParticleHPThermalScattering::BuildPhysicsTable(const G4ParticleDefinition& particle) {
   buildPhysicsTable();
   theHPElastic->BuildPhysicsTable( particle );
}


std::map < G4double , std::vector < std::pair< G4double , G4double >* >* >* G4SaG4nParticleHPThermalScattering::readACoherentFSDATA( G4String name )
{
   ...
   G4SaG4nParticleHPManager::GetInstance()->GetDataStream(name,theChannel);
...
std::map < G4double , std::vector < E_P_E_isoAng* >* >* G4SaG4nParticleHPThermalScattering::readAnInelasticFSDATA ( G4String name )
{
...
E_P_E_isoAng* G4SaG4nParticleHPThermalScattering::readAnE_P_E_isoAng( std::istream* file ) // for inelastic
{
   ...
   *file >> nl;
   ...
   for ( G4int i = 0 ; i < aData->n ; i++ )
   {
      ...
      *file >> prob;
      ...
      //G4cout << "G4SaG4nParticleHPThermalScattering inelastic " << energy/eV << " " <<  i << " " << prob << " " << aData->prob[ i ] << G4endl; 
      for ( G4int j = 0 ; j < anE_isoAng->n ; j++ )
      {
         G4double x;
         *file >> x;
         anE_isoAng->isoAngle[j] = x ;
         //G4cout << "G4SaG4nParticleHPThermalScattering inelastic " << x << anE_isoAng->isoAngle[j] << G4endl; 
      }
   } 
...
std::map < G4double , std::vector < E_isoAng* >* >* G4SaG4nParticleHPThermalScattering::readAnIncoherentFSDATA ( G4String name )
{
...
E_isoAng* G4SaG4nParticleHPThermalScattering::readAnE_isoAng( std::istream* file )
{
...
G4HadFinalState* G4SaG4nParticleHPThermalScattering::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& aNucleus )
{
...
//--------------------------------------------------
std::pair< G4double , G4int> G4SaG4nParticleHPThermalScattering::
...
// New method added by L. Thulliez 2021 (CEA-Saclay)
//--------------------------------------------------
std::pair< G4double , G4double > G4SaG4nParticleHPThermalScattering::
...
// New method added by L. Thulliez 2021 (CEA-Saclay)
//--------------------------------------------------
G4double G4SaG4nParticleHPThermalScattering::getMu( G4double rndm1, G4double rndm2, E_isoAng* anEPM )
{
   G4double result = 0.0;
...
G4double G4SaG4nParticleHPThermalScattering::getMu( E_isoAng* anEPM  )
{
...
std::pair < G4double , G4double >  G4SaG4nParticleHPThermalScattering::find_LH ( G4double x , std::vector< G4double >* aVector )
{
   G4double LL = 0.0; 
   G4double H = 0.0; 
...
G4double G4SaG4nParticleHPThermalScattering::get_linear_interpolated ( G4double x , std::pair< G4double , G4double > Low , std::pair< G4double , G4double > High )
{ 
   G4double y=0.0;
   if ( High.first - Low.first != 0 ) {
      ...
      if ( High.second == Low.second ) {
         ...
         G4cout << "G4SaG4nParticleHPThermalScattering liner interpolation err!!" << G4endl; 
      }
   }
...
E_isoAng
G4SaG4nParticleHPThermalScattering::create_E_isoAng_from_energy(G4double energy,
                                                           std::vector<E_isoAng*>* vEPM)
{
   ...
   if (panEPM_T_EL != 0 && panEPM_T_EH != 0) {
     ...
     if (panEPM_T_EL->n == panEPM_T_EH->n) {
       ...
       G4Exception("G4SaG4nParticleHPThermalScattering::create_E_isoAng_from_energy",
                   "NotSupported", JustWarning,
                   "G4SaG4nParticleHPThermalScattering does not support yet EL->n != EH->n."); 
     }
...
G4double G4SaG4nParticleHPThermalScattering::get_secondary_energy_from_E_P_E_isoAng ( G4double random , E_P_E_isoAng* anE_P_E_isoAng )
{
...
std::pair< G4double , E_isoAng > G4SaG4nParticleHPThermalScattering::create_sE_and_EPM_from_pE_and_vE_P_E_isoAng ( G4double rand_for_sE ,  G4double pE , std::vector < E_P_E_isoAng* >*  vNEP_EPM )
{
   std::map< G4double , G4int > map_energy;
   map_energy.clear();
   std::vector< G4double > v_energy;
...
void G4SaG4nParticleHPThermalScattering::buildPhysicsTable()
{
...
G4int G4SaG4nParticleHPThermalScattering::getTS_ID ( const G4Material* material , const G4Element* element )
{
   G4int result = -1;
...
const std::pair<G4double, G4double> G4SaG4nParticleHPThermalScattering::GetFatalEnergyCheckLevels() const
{
...
void G4SaG4nParticleHPThermalScattering::AddUserThermalScatteringFile( G4String nameG4Element , G4String filename)
{
...
G4bool G4SaG4nParticleHPThermalScattering::check_E_isoAng( E_isoAng* anE_IsoAng )
{
   G4bool result=false;
...
void G4SaG4nParticleHPThermalScattering::ModelDescription(std::ostream& outFile) const
{
