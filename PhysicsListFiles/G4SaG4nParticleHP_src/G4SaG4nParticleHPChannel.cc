...
G4double G4SaG4nParticleHPChannel::GetFSCrossSection(G4double energy, G4int isoNumber)
{
  ...
  void G4SaG4nParticleHPChannel::Init(G4Element * anElement, const G4String dirName)  
  {
    theDir = dirName;
    theElement = anElement;
  }
  
  G4bool G4SaG4nParticleHPChannel::Register(G4SaG4nParticleHPFinalState *theFS)
  {
	registerCount++;
...
*/
    ...
    theIsotopeWiseData = new G4SaG4nParticleHPIsoData [niso];
    delete [] active;
    active = new G4bool[niso];

    delete [] theFinalStates;
    theFinalStates = new G4SaG4nParticleHPFinalState * [niso];
  ...
  //void G4SaG4nParticleHPChannel::UpdateData(G4int A, G4int Z, G4int index, G4double abundance)
  void G4SaG4nParticleHPChannel::UpdateData(G4int A, G4int Z, G4int M, G4int index, G4double abundance, G4ParticleDefinition* projectile)
  {
  ...
  void G4SaG4nParticleHPChannel::Harmonise(G4SaG4nParticleHPVector *& theStore, G4SaG4nParticleHPVector * theNew)
  {
    G4int s_tmp = 0, n=0, m_tmp=0;
    G4SaG4nParticleHPVector * theMerge = new G4SaG4nParticleHPVector;
    G4SaG4nParticleHPVector * anActive = theStore;
    G4SaG4nParticleHPVector * aPassive = theNew;
    G4SaG4nParticleHPVector * tmp;
    G4int a = s_tmp, p = n, t;
...
#include "G4SaG4nParticleHPThermalBoost.hh"
  ...
  G4HadFinalState * G4SaG4nParticleHPChannel::
  ApplyYourself(const G4HadProjectile & theTrack, G4int anIsotope)
  {
//    G4cout << "G4SaG4nParticleHPChannel::ApplyYourself+"<<niso<<G4endl;
    ...
    if(sum == 0) 
    {
//      G4cout << "G4SaG4nParticleHPChannel::ApplyYourself theFinalState->Initialize+"<<G4endl;
//      G4cout << "G4SaG4nParticleHPChannel::ApplyYourself theFinalState->Initialize-"<<G4endl;
...
//      G4cout << "TESTHP 23 NISO="<<niso<<G4endl;
      ...
      G4double running=0;
//      G4cout << "G4SaG4nParticleHPChannel::ApplyYourself Done the sum"<<niso<<G4endl;
...
//	      G4cout << "TESTHP 24 it="<<it<<G4endl;
    ...
    //G4cout << "TK G4SaG4nParticleHPChannel Elastic, Capture and Fission Cases "
    //<< " Z= " << this->GetZ(it) << " A = " << this->GetN(it) << G4endl;
    G4SaG4nParticleHPManager::GetInstance()->GetReactionWhiteBoard()->SetTargA( A );
    G4SaG4nParticleHPManager::GetInstance()->GetReactionWhiteBoard()->SetTargZ( Z );
    G4SaG4nParticleHPManager::GetInstance()->GetReactionWhiteBoard()->SetTargM( M );

    return theFinalState;
  }


void G4SaG4nParticleHPChannel::DumpInfo(){
