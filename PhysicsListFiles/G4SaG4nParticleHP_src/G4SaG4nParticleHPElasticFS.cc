...
G4SaG4nParticleHPElasticFS::G4SaG4nParticleHPElasticFS()
{
...
void G4SaG4nParticleHPElasticFS::Init(G4double A, G4double Z, G4int M,
                                 G4String& dirName, G4String&,
                                 G4ParticleDefinition* )
{
  G4String tString = "/FS";
  G4bool dbool;
  G4SaG4nParticleHPDataUsed aFile =
  ...
  if (repFlag == 1) {
    G4int nEnergy;
    theData >> nEnergy; 
    theCoefficients = new G4SaG4nParticleHPLegendreStore(nEnergy);
    ...
    G4int nEnergy;
    theData >> nEnergy;
    theProbArray = new G4SaG4nParticleHPPartial(nEnergy, nEnergy);
    ...
    if (nEnergy_Legendre <= 0 ) {
      ...
      iss << "G4SaG4nParticleHPElasticFS::Init Data Error repFlag is 3 but nEnergy_Legendre <= 0";
    ...
    theCoefficients = new G4SaG4nParticleHPLegendreStore( nEnergy_Legendre );
    theCoefficients->InitInterpolation( theData );
    G4double temp, energy;
    G4int tempdep, nLegendre;
    ...
    theProbArray = new G4SaG4nParticleHPPartial( nEnergy_Prob , nEnergy_Prob );
    theProbArray->InitInterpolation( theData );
    G4int nPoints;
    for (G4int i = 0; i < nEnergy_Prob; i++) {
      ...
      throw G4HadronicException(__FILE__, __LINE__, "G4SaG4nParticleHPElasticFS::Init -- unusable number for repFlag");
  }
...
G4HadFinalState*
G4SaG4nParticleHPElasticFS::ApplyYourself(const G4HadProjectile& theTrack)
{  
  ...
  if (repFlag == 1) {
    ...
    throw G4HadronicException(__FILE__, __LINE__,
                              "G4SaG4nParticleHPElasticFS::Init -- unusable number for repFlag");
  }
  ...
  if (frameFlag == 1) {
    ...
    throw G4HadronicException(__FILE__, __LINE__,
                   "G4SaG4nParticleHPElasticFS::ApplyYourSelf frameflag incorrect");
  }
