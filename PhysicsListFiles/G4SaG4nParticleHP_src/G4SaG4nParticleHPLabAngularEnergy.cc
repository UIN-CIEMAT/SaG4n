...
void G4SaG4nParticleHPLabAngularEnergy::Init(std::istream & aDataFile)
{
  aDataFile >> nEnergies;
  theManager.Init(aDataFile);
  theEnergies = new G4double[nEnergies];
  nCosTh = new G4int[nEnergies];
  theData = new G4SaG4nParticleHPVector * [nEnergies];
  ...
  for(G4int i=0; i<nEnergies; i++)
  {
    ...
    theData[i] = new G4SaG4nParticleHPVector[nCosTh[i]];
    G4double label;
...
G4ReactionProduct * G4SaG4nParticleHPLabAngularEnergy::Sample(G4double anEnergy, G4double massCode, G4double )
{
   ...
   {
     throw G4HadronicException(__FILE__, __LINE__, "G4SaG4nParticleHPLabAngularEnergy: Unknown ion case 2");
   }
   ...
   if ( it == 0 ) // it marks the energy bin --> we do not extrapolate to low energies, we extrapolate to high energies (??)
   {
     ...
     //---------------------------------------------------------
     //CosTheta:
     G4SaG4nParticleHPVector theThVec;
     ...
     G4SaG4nParticleHPVector theBuff1a;
     ...
     G4SaG4nParticleHPVector theBuff2a;
   ...
   else // this is the small big else.
   {
     ...
     x1 = theEnergies[it-1];
     x2 = theEnergies[it];
     G4SaG4nParticleHPVector thBuff1; // to be interpolated as run1.
     ...
     G4SaG4nParticleHPVector thBuff2;
     ...
     */
     ...
     G4SaG4nParticleHPVector theStore1;
     ...
     G4SaG4nParticleHPVector theBuff1b;
     ...
     G4SaG4nParticleHPVector theBuff2b;
     ...
     G4SaG4nParticleHPVector theStore2;
     ...
     x1 = theEnergies[it-1];
     x2 = theEnergies[it];
     G4SaG4nParticleHPVector theOne1;
     ...
     G4SaG4nParticleHPVector theOne2;
     ...
     G4SaG4nParticleHPVector theOne;
