...
G4SaG4nParticleHPCaptureFS::G4SaG4nParticleHPCaptureFS()
  {
  ...
  G4HadFinalState * G4SaG4nParticleHPCaptureFS::ApplyYourself(const G4HadProjectile & theTrack)
  {
...
// prepare neutron
    ...
    if ( HasFSData() && !G4SaG4nParticleHPManager::GetInstance()->GetUseOnlyPhotoEvaporation() ) 
    { 
...
//Add photons
       ...
       {
              //G4cout << "TKDB G4SaG4nParticleHPCaptureFS::ApplyYourself we will create additional " << nPhotons - nNonZero << " photons" << G4endl;
...
#include <sstream> 
  void G4SaG4nParticleHPCaptureFS::Init (G4double A, G4double Z, G4int M, G4String & dirName, G4String &, G4ParticleDefinition* )
  {
   ...
   G4SaG4nParticleHPManager::GetInstance()->GetDataStream(filenameMF6,theData);
   ...
   if ( theData.good() == true ) { 
    ...
    G4String tString = "/FS";
    G4bool dbool;
    G4SaG4nParticleHPDataUsed aFile = theNames.GetName(static_cast<G4int>(A), static_cast<G4int>(Z), M, dirName, tString, dbool);
