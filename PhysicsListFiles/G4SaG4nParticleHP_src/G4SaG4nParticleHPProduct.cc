...
G4int G4SaG4nParticleHPProduct::GetMultiplicity(G4double anEnergy )
{
  ...
  //g  G4cout << "G4SaG4nParticleHPProduct MEAN NUMBER OF PARTICLES " << mean << " for " << theMass << G4endl;
  //if( mean <= 0. ) return 0;
  //151120 TK Modified for solving reproducibility problem 
...
#ifdef G4PHPDEBUG
     #ifdef G4VERBOSE
     if( std::getenv("G4SaG4nParticleHPDebug") && G4HadronicParameters::Instance()->GetVerboseLevel() > 0 )
       G4cout << "G4SaG4nParticleHPProduct::GetMultiplicity " << theMassCode << " " << theMass << " multi " << multi << " mean " << mean << G4endl;
     #endif
#endif
  }
...
G4ReactionProductVector * G4SaG4nParticleHPProduct::Sample(G4double anEnergy, G4int multi)
{
  if(theDist == 0) { return 0; }
...
//                  - theActualStateQValue;
 ...
 if( std::getenv("G4PHPTEST") )
    #ifdef G4VERBOSE
    if( std::getenv("G4SaG4nParticleHPDebug") && tmp != 0 && G4HadronicParameters::Instance()->GetVerboseLevel() > 0 )
      G4cout << multi << " " << i << " @@@ G4SaG4nParticleHPProduct::Sample " << anEnergy << " Mass " << theMassCode << " " << theMass << G4endl;
    #endif
#endif
    ...
    #ifdef G4VERBOSE
    if( std::getenv("G4SaG4nParticleHPDebug") && tmp != 0 && G4HadronicParameters::Instance()->GetVerboseLevel() > 0 )
      G4cout << multi << " " << i << " @@@ G4SaG4nParticleHPProduct::Sample " << tmp->GetDefinition()->GetParticleName() << " E= " << tmp->GetKineticEnergy() << G4endl;
    #endif
#endif
  }
  if(multi == 0) 
